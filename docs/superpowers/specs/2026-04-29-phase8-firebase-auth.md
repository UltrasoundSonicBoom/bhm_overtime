...
>   런타임 키는 `snuhmate_*`, `_guest`, `_uid_{uid}` 기준만 지원한다.
...
| localStorage key | 데이터 | Firestore path |
|---|---|---|
| `leaveRecords` | 휴가 기록 배열 | `users/{uid}/leave/{yyyy}` 1 doc per 년 (4-seg). `entries`, `annualBalance` |
...
| 근무이력 (`snuhmate_work_history`) | **work_history** | `users/{uid}/work_history/{entryId}` |
...
`snuhmate_*`, `_guest`, `_uid_{uid}`, prefix 없음 (`overtimeRecords`, `otManualHourly`, `leaveRecords`) 기준으로 중앙화. old BHM-prefixed keys are unsupported:
...
### 4.3 legacy BHM-prefixed key unsupported 검증

- old BHM-prefixed localStorage keys are no longer read, written, or migrated.
- ✅ `snuhmate_settings`, `snuhmate_local_uid`, `snuhmate_device_id`, `snuhmate_anon_id`, `snuhmate_demo_mode`, `snuhmate_debug_parser`, `snuhmate_leave_migrated_v1`, `snuhmate_last_edit_*` 만 런타임 지원
- ✅ `snuhmate_hr_profile` → `profile.js` 저장/로드 기준
- ✅ `snuhmate_work_history`, `snuhmate_work_history_seeded` → `work-history.js` 저장/로드 기준
- ✅ 정적 회귀 가드: old BHM-prefixed storage prefix 및 old global config name 금지
