...
function initBrowse() {
  ...
  // 정적 JSON(union_regulation_2026.json)이 현재 찾아보기의 표준 데이터 소스.
  // JSON 로드 실패 시에만 API(/regulations/browse) fallback 시도.
  // (DB는 현재 주제 기반 구조로, 장(Chapter) 기반 UI와 맞지 않아 후순위)
  tryLoadBrowseFromJson().then(function() {
    if (!DATA.handbook || DATA.handbook.length === 0) {
      return tryLoadBrowseFromApi();
    }
  }).then(function() {
...
function searchChatLocal(query) {
  ...
  // 물어보기 탭 로컬 검색 fallback: 주제 기반 DATA.handbookTopics 사용
  // (DATA.handbook은 찾아보기 탭용 장(Chapter) 구조로 교체됨)
  var topicsSource = (DATA.handbookTopics && DATA.handbookTopics.length > 0)
    ? DATA.handbookTopics
    : DATA.handbook;

  if (mappedCategory && topicsSource) {
    var topicSection = topicsSource.find(function(h) { return h.category === mappedCategory; });
    if (topicSection) {
      var articleScores = topicSection.articles.map(function(art) {
        var s = 0;
      ...
      var sortedTopics = articleScores.sort(function(a, b) { return b.score - a.score; });
      if (qWords.length >= 2 && sortedTopics[0] && sortedTopics[0].score > 0) {
        handbookArticles = sortedTopics.filter(function(a) { return a.score > 0; }).slice(0, 2);
      } else {
        handbookArticles = sortedTopics;
      }
    }
  }

  if (handbookArticles.length === 0 && best && topicsSource) {
    var faqCatMap = {
    ...
    if (hbCat) {
      var sect = topicsSource.find(function(h) { return h.category === hbCat; });
      if (sect) {
