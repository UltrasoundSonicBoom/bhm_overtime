// Auth 제거됨 (2026-04-21). UI/렌더 구조는 유지. 향후 재도입 시 참고용.
const API_BASE = (() => {
...
const els = {
  // loginBtn, authStatus 제거됨 (2026-04-21)
...
async function apiJson(path, options = {}, _requireAuth = false) {
  // auth 제거됨. _requireAuth 인자는 호출부 호환 위해 보존 (무시됨).
...
// updateAuthState() 제거됨 (2026-04-21). auth 없이 항상 작업 가능 상태.
...
async function loadFaqs() {
  ...
  try {
    const data = await apiJson(`/admin/faqs?versionId=${state.selectedVersionId}`);
...
async function loadContentEntries() {
  try {
    const data = await apiJson('/admin/content');
...
async function loadApprovals() {
  try {
    const data = await apiJson('/admin/approvals');
...
async function loadAuditLogs() {
  try {
    const data = await apiJson('/admin/audit-logs');
...
async function loadAdminData() {
  // auth 없이 호출. 서버 없으면 catch 로 조용히 빈 state 유지.
  try {
    const [dashboardData, versionsData, snapshotsData] = await Promise.all([
      apiJson('/admin/dashboard'),
      apiJson('/admin/versions'),
      apiJson(`/admin/calendar/snapshots?year=${state.year}`),
    ]);
...
// loginBtn 핸들러 제거됨 (2026-04-21). auth 없이 운영.
...
async function initApp() {
  // auth 제거됨 (2026-04-21). 바로 workspace 로드.
  await loadWorkspace();
}
