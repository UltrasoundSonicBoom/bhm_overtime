...
window.AppLock = (function () {
  ...
  function setupPin(pin) {
    ...
    return _sha256(pin + salt).then(function (hash) {
      ...
      _unlocked = true;
      // Drive 백업 (SyncManager 로드된 경우)
      if (window.SyncManager && typeof window.SyncManager.pushAppLockSettings === 'function') {
        window.SyncManager.pushAppLockSettings();
      }
    });
  }
  ...
  function resetViaReauth(onSuccess) {
    ...
    if (!settings.googleSub) {
      alert('Google 계정으로 로그인된 상태에서만 PIN을 재설정할 수 있어요.\nGuest 모드에서는 브라우저 데이터를 초기화해야 합니다.');
      return;
    }
    window._appLockReauthPending = true;
    window._appLockReauthCallback = onSuccess;
  ...
  function _renderOverlay() {
    ...
    // PIN 분실
    if (settings.googleSub) {
      var forgotBtn = _el('button', {
        id: 'alForgotBtn',
        css: 'background:none;border:none;color:var(--text-muted,#777);font-size:0.8rem;cursor:pointer;text-decoration:underline;padding:4px;',
        text: 'PIN을 잊으셨나요?'
      });
      inner.appendChild(forgotBtn);
    } else {
      inner.appendChild(_el('p', {
  ...
  var BiometricLock = (function () {
    ...
    function register() {
      ...
      var settings = _loadSettings();
      var userId = new TextEncoder().encode(settings.googleSub || 'snuhmate-guest');
      ...
      return navigator.credentials.create({
        publicKey: {
          ...
          user: {
            id: userId,
            name: settings.googleEmail || 'user',
            displayName: settings.googleName || 'SNUH 메이트 사용자'
          },
