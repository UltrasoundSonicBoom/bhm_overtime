window.UNION_REGULATION_CAL_2026 = {
  "_meta": {
    "source": "union_regulation_2026.json",
    "version": "2026.1.0-generated",
    "reference_shape": "nurse_regulation.json"
  },
  "fixed_allowances": {
    "meal_subsidy": {
      "amount": 150000,
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "제58조(복리후생시설)",
          "clause": "(3) 급식보조비 : 매월 150,000원을 지급하되 상황에 따라 인상할 수 있다."
        }
      ]
    },
    "transport_subsidy": {
      "amount": 150000,
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "제58조(복리후생시설)",
          "clause": "(4) 교통보조비 : 병원은 매월 일정액의 교통보조비를 전 조합원에게 지급한다."
        }
      ]
    },
    "military_service_pay": {
      "amount": 45000,
      "max_months": 24,
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "군복무수당에 관한 사항",
          "clause": "병원은 2015.07.01 이후 입사자의 군복무수당 지급 시 본인의 군복무기간에 대해 월할 계산하여 지급한다. (2024년 1월부터 시행)"
        }
      ]
    },
    "training_allowance": {
      "amount": 40000,
      "refs": [
        {
          "source": "nurse_regulation.json",
          "path": "wage_structure_and_allowances.fixed_allowances.training_monthly"
        }
      ]
    },
    "refresh_support_allowance": {
      "monthly_amount": 30000,
      "annual_amount": 360000,
      "effective_date": "2026-01-01",
      "refs": [
        {
          "source": "nurse_regulation.json",
          "path": "scenarios.refresh-support.expected.annual_support"
        }
      ]
    }
  },
  "formulas": {
    "holiday_bonus_event": {
      "expression": "(base_salary + adjust_pay * 0.5) * 0.5",
      "payment_months": [
        "lunar_new_year_month",
        "chuseok_month",
        5,
        7
      ],
      "refs": [
        {
          "source": "nurse_regulation.json",
          "path": "wage_structure_and_allowances.conditional_wages.holiday_support"
        }
      ]
    }
  },
  "ordinary_wage": {
    "included_variable_keys": [
      "base_salary",
      "seniority_base_salary",
      "military_service_pay",
      "ability_pay",
      "bonus_monthly",
      "family_support_pay",
      "adjust_pay",
      "upgrade_adjust_pay",
      "long_service_pay",
      "special_duty_pay",
      "position_pay",
      "work_support_pay",
      "meal_subsidy",
      "transport_subsidy",
      "holiday_bonus_event",
      "training_allowance",
      "refresh_support_allowance"
    ],
    "refs": [
      {
        "source": "nurse_regulation.json",
        "path": "wage_structure_and_allowances.wage_components.ordinary_wage"
      }
    ]
  },
  "annual_leave": {
    "unused_compensation_rule": "(4) 연차휴가는 본인의 청구에 의하여 필요한 시기에 분할 사용할 수 있으며, 사용하지 않은 일수에 대해서는 다음해 1월에 평균임금(법정수당, 연차보전수당, 야간근무가산금은 제외)을 지급한다.",
    "refs": [
      {
        "source": "union_regulation_2026",
        "title": "제36조(연차 유급휴가)",
        "clause": "(4) 연차휴가는 본인의 청구에 의하여 필요한 시기에 분할 사용할 수 있으며, 사용하지 않은 일수에 대해서는 다음해 1월에 평균임금(법정수당, 연차보전수당, 야간근무가산금은 제외)을 지급한다."
      }
    ]
  },
  "wage_tables": {
    "general_m_grade": {
      "M3": {
        "base_salary_by_year": [
          54482400,
          54944400,
          55411200,
          55874400,
          56349600,
          56824800,
          57310800,
          57796800
        ],
        "ability_pay": 22734000,
        "bonus": 2908800,
        "family_support": 15941330
      },
      "M2": {
        "base_salary_by_year": [
          51369600,
          51798000,
          52226400,
          52666800,
          53106000,
          53545200,
          53996400,
          54446400
        ],
        "ability_pay": 21274800,
        "bonus": 2670000,
        "family_support": 14619150
      },
      "M1": {
        "base_salary_by_year": [
          47559600,
          47956800,
          48354000,
          48750000,
          49155600,
          49562400,
          49981200,
          50401200
        ],
        "ability_pay": 19502400,
        "bonus": 2448000,
        "family_support": 13387470
      }
    },
    "general_s_grade": {
      "S3": {
        "base_salary_by_year": [
          44073600,
          44500800,
          44928000,
          45370800,
          45814800,
          46255200,
          46706400,
          47157600
        ],
        "ability_pay": 16208400,
        "bonus": 2366400,
        "family_support": 12932980
      },
      "S2": {
        "base_salary_by_year": [
          40173600,
          40568400,
          40966800,
          41361600,
          41778000,
          42183600,
          42589200,
          43008000
        ],
        "ability_pay": 14558400,
        "bonus": 2127600,
        "family_support": 11610790
      },
      "S1": {
        "base_salary_by_year": [
          36316800,
          36669600,
          37030800,
          37398000,
          37767600,
          38128800,
          38505600,
          38880000
        ],
        "ability_pay": 12908400,
        "bonus": 1888800,
        "family_support": 10284680
      }
    },
    "general_j_grade": {
      "J3": {
        "base_salary_by_year": [
          32379600,
          32697600,
          33019200,
          33340800,
          33666000,
          33988800,
          34318800,
          34652400
        ],
        "ability_pay": 8965200,
        "bonus": 1880400,
        "family_support": 10237460
      },
      "J2": {
        "base_salary_by_year": [
          28262400,
          28542000,
          28816800,
          29096400,
          29390400,
          29680800,
          29967600,
          30258000
        ],
        "ability_pay": 7569600,
        "bonus": 1588800,
        "family_support": 8622110
      },
      "J1": {
        "base_salary_by_year": [
          25752000,
          26008800,
          26260800,
          26516400,
          26779200,
          27049200,
          27314400,
          27583200
        ],
        "ability_pay": 6710400,
        "bonus": 1410000,
        "family_support": 7634410
      }
    },
    "operation_l_grade": {
      "L3": {
        "base_salary_by_year": [
          41824800,
          42169200,
          42511200,
          42853200,
          43206000,
          43556400,
          43914000,
          44274000
        ],
        "ability_pay": 9142800,
        "bonus": 1899600,
        "family_support": 10361870
      },
      "L2": {
        "base_salary_by_year": [
          38842800,
          39156000,
          39466800,
          39788400,
          40107600,
          40424400,
          40756800,
          41084400
        ],
        "ability_pay": 8671200,
        "bonus": 1744800,
        "family_support": 9502450
      },
      "L1": {
        "base_salary_by_year": [
          36064800,
          36350400,
          36640800,
          36930000,
          37227600,
          37522800,
          37826400,
          38128800
        ],
        "ability_pay": 8202000,
        "bonus": 1600800,
        "family_support": 8701860
      }
    },
    "operation_c_grade": {
      "C3": {
        "base_salary_by_year": [
          33271200,
          33600000,
          33931200,
          34266000,
          34612800,
          34948800,
          35294400,
          35640000
        ],
        "ability_pay": 7279200,
        "bonus": 1546800,
        "family_support": 8406440
      },
      "C2": {
        "base_salary_by_year": [
          30451200,
          30757200,
          31063200,
          31368000,
          31690800,
          32007600,
          32319600,
          32642400
        ],
        "ability_pay": 6770400,
        "bonus": 1392000,
        "family_support": 7547010
      },
      "C1": {
        "base_salary_by_year": [
          27662400,
          27936000,
          28220400,
          28502400,
          28794000,
          29074800,
          29366400,
          29656800
        ],
        "ability_pay": 6249600,
        "bonus": 1237200,
        "family_support": 6685040
      }
    },
    "operation_a_grade": {
      "A3": {
        "base_salary_by_year": [
          24810000,
          25060800,
          25310400,
          25564800,
          25825200,
          26073600,
          26335200,
          26593200
        ],
        "ability_pay": 5545200,
        "bonus": 1231200,
        "family_support": 6654350
      },
      "A2": {
        "base_salary_by_year": [
          21835200,
          22056000,
          22276800,
          22495200,
          22732800,
          22962000,
          23187600,
          23421600
        ],
        "ability_pay": 4929600,
        "bonus": 1041600,
        "family_support": 5604370
      },
      "A1": {
        "base_salary_by_year": [
          20258400,
          20463600,
          20665200,
          20871600,
          21085200,
          21297600,
          21508800,
          21724800
        ],
        "ability_pay": 4371600,
        "bonus": 926400,
        "family_support": 4962370
      }
    },
    "environment_sl_grade": {
      "SL3": {
        "base_salary_by_year": [
          41709600,
          42019200,
          42328800,
          42638400,
          42956400,
          43272000,
          43596000,
          43921200
        ],
        "ability_pay": 8229600,
        "bonus": 1710000,
        "family_support": 2548900
      },
      "SL2": {
        "base_salary_by_year": [
          38683200,
          38966400,
          39244800,
          39535200,
          39824400,
          40112400,
          40407600,
          40706400
        ],
        "ability_pay": 7804800,
        "bonus": 1570800,
        "family_support": 2411400
      },
      "SL1": {
        "base_salary_by_year": [
          35863200,
          36120000,
          36381600,
          36642000,
          36909600,
          37177200,
          37452000,
          37726800
        ],
        "ability_pay": 7382400,
        "bonus": 1441200,
        "family_support": 2283800
      }
    },
    "environment_sc_grade": {
      "SC3": {
        "base_salary_by_year": [
          33208800,
          33507600,
          33808800,
          34108800,
          34419600,
          34725600,
          35036400,
          35349600
        ],
        "ability_pay": 6552000,
        "bonus": 1393200,
        "family_support": 2236500
      },
      "SC2": {
        "base_salary_by_year": [
          30326400,
          30606000,
          30880800,
          31155600,
          31447200,
          31730400,
          32014800,
          32306400
        ],
        "ability_pay": 6093600,
        "bonus": 1252800,
        "family_support": 2099000
      },
      "SC1": {
        "base_salary_by_year": [
          27470400,
          27720000,
          27976800,
          28232400,
          28494000,
          28748400,
          29010000,
          29272800
        ],
        "ability_pay": 5625600,
        "bonus": 1113600,
        "family_support": 1961500
      }
    },
    "environment_sa_grade": {
      "SA3": {
        "base_salary_by_year": [
          24872400,
          25099200,
          25326000,
          25552800,
          25788000,
          26017200,
          26250000,
          26485200
        ],
        "ability_pay": 4990800,
        "bonus": 1108800,
        "family_support": 1956000
      },
      "SA2": {
        "base_salary_by_year": [
          21777600,
          21976800,
          22176000,
          22374000,
          22590000,
          22792800,
          22998000,
          23206800
        ],
        "ability_pay": 4437600,
        "bonus": 938400,
        "family_support": 1788800
      },
      "SA1": {
        "base_salary_by_year": [
          20097600,
          20283600,
          20469600,
          20653200,
          20842800,
          21037200,
          21228000,
          21421200
        ],
        "ability_pay": 3934800,
        "bonus": 834000,
        "family_support": 1686500
      }
    }
  },
  "petition_leave_and_congrats": {
    "marriage_self": {
      "leave_days": 5,
      "hospital_amount": 300000,
      "pension_support": "",
      "coop_support": "결혼 축하 선물",
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "청원휴가 및 경조금 표",
          "row_title": "본인 결혼"
        }
      ]
    },
    "marriage_child": {
      "leave_days": 1,
      "hospital_amount": 100000,
      "pension_support": "",
      "coop_support": "",
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "청원휴가 및 경조금 표",
          "row_title": "자녀 결혼"
        }
      ]
    },
    "childbirth_spouse": {
      "leave_days": 20,
      "hospital_amount": 100000,
      "pension_support": "",
      "coop_support": "첫째·둘째: 10만원, 셋째 이상: 30만원",
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "청원휴가 및 경조금 표",
          "row_title": "배우자 출산"
        }
      ]
    },
    "death_self": {
      "leave_days": "없음",
      "hospital_amount": 1000000,
      "pension_support": "사망조위금 지급",
      "coop_support": "경조비 지급",
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "청원휴가 및 경조금 표",
          "row_title": "본인 사망"
        }
      ]
    },
    "death_spouse": {
      "leave_days": "없음",
      "hospital_amount": 1000000,
      "pension_support": "사망조위금 지급",
      "coop_support": "경조비 지급",
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "청원휴가 및 경조금 표",
          "row_title": "배우자 사망"
        }
      ]
    },
    "death_parents": {
      "leave_days": 5,
      "hospital_amount": 300000,
      "pension_support": "사망조위금 지급",
      "coop_support": "경조비 지급",
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "청원휴가 및 경조금 표",
          "row_title": "(본인, 배우자)의 부모 사망"
        }
      ]
    },
    "death_child_and_spouse": {
      "leave_days": 3,
      "hospital_amount": 300000,
      "pension_support": "",
      "coop_support": "",
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "청원휴가 및 경조금 표",
          "row_title": "자녀 및 그 배우자 사망"
        }
      ]
    },
    "death_grandparents": {
      "leave_days": 3,
      "hospital_amount": 50000,
      "pension_support": "",
      "coop_support": "",
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "청원휴가 및 경조금 표",
          "row_title": "(본인, 배우자)의 조부모, 외조부모 사망"
        }
      ]
    },
    "death_siblings": {
      "leave_days": 3,
      "hospital_amount": 50000,
      "pension_support": "",
      "coop_support": "",
      "refs": [
        {
          "source": "union_regulation_2026",
          "title": "청원휴가 및 경조금 표",
          "row_title": "(본인, 배우자)의 형제, 자매 사망"
        }
      ]
    }
  }
};
