...
**Acceptance criteria:**
- [ ] 개인정보 저장 후 CTA 카드 표시: "⏰ [직종별 문구] →" 버튼
- [ ] 버튼 클릭 시 해당 탭으로 이동하고 CTA가 사라짐
- [ ] 탭 이동 시 (`switchTab()`) CTA 자동 숨김
- [ ] 직종별 4가지 문구 정확히 분기됨
- [ ] 재저장 시에도 매번 CTA 표시 (중복 저장 케이스 포함)
...
**Acceptance criteria:**
- [ ] 시간외 탭 진입 시 프로필 미저장이면 노란 배너 표시
- [ ] 배너 내 버튼 클릭 → 개인정보 탭으로 이동
- [ ] 프로필 저장 후 시간외 탭 재방문 시 배너 미표시 (시급 > 0)
- [ ] 기존 `hourlyRate` 계산 로직 회귀 없음
...
**Acceptance criteria:**
- [ ] 홈 탭 초기 진입 시 프로필 미저장이면 힌트 배너 표시
- [ ] 힌트 내 "지금 입력 →" 버튼 클릭 → 개인정보 탭 이동
- [ ] 프로필 저장 완료 후 힌트 사라짐 (페이지 리로드 없이)
- [ ] 프로필 저장된 상태에서 홈 탭 진입 시 힌트 미표시
...
- [ ] F1: 저장 직후 CTA 카드 표시 + 직종별 문구 분기 확인
- [ ] F2: 시간외 탭 노란 배너 + 버튼 확인
- [ ] F3: 홈 탭 힌트 배너 표시/숨김 확인
- [ ] 기존 계산 기능 회귀 없음 (saveProfile, switchTab, hourlyRate)
- [ ] 30일 후 GA에서 시간외 views 및 전환율 측정
