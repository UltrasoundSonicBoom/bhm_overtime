...
function _applySnapshotToUidLocal(uid, selectedIds, snapshot) {
  ...
  try {
    ...
    if (selectedIds.includes('leave') && flat.leaveRecords) {
      localStorage.setItem('leaveRecords', JSON.stringify(flat.leaveRecords));
    }
...
export async function uploadCategories(uid, selectedIds, snapshot = null) {
  ...
  if (selectedIds.includes('leave')) {
    syncTasks.push({
      ...
      label: '휴가',
      run: async () => {
        const data = _snapshotValue(source, 'leaveRecords');
        if (!data) return;
    ...
    syncTasks.push({
      ...
      run: async () => {
        ...
        await Promise.all(Object.entries(source.payslips || {}).map(([key, data]) => {
          ...
          return writePayslip(null, uid, payMonth, data);
        }));
      },
    });
  }
