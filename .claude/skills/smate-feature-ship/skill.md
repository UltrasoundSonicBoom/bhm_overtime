...
### Step 5-V: Phase 완료 검증 (구현 Phase마다 반드시 실행)

각 구현 Phase가 끝날 때마다 아래 4단계를 순서대로 실행한다. 하나라도 ❌이면 다음 Phase로 넘어가지 않는다.

#### 1. Unit Test — 순수 로직 검증
```bash
# 백엔드 변경 시
cd backend && poetry run pytest tests/<해당_모듈>/ -v

# 프런트엔드 변경 시
pnpm test:unit
```

#### 2. 생성 파일·DB·JSON 확인 — 산출물 shape 검증
| 대상 | 확인 명령 | 기대 결과 |
|------|----------|----------|
| 생성 파일 | `ls -la <파일 경로>` | 존재 + 0 바이트 아님 |
| Firestore 문서 | `firebase firestore:get <col>/<docId>` | 필수 필드·타입 일치 |
| JSON 출력 | `cat <output.json> \| jq 'keys'` | 필수 키 전부 존재 |

생성물이 없거나 shape이 틀리면 구현 미완성으로 간주한다.

#### 3. Endpoint 연결 확인 — API 응답 검증
```bash
# FastAPI 서버 실행 중 상태에서
curl -s http://localhost:8000/<endpoint> | jq .
# POST 엔드포인트:
curl -s -X POST http://localhost:8000/<endpoint> \
  -H "Content-Type: application/json" -d '<payload>' | jq .status
```
HTTP 200 + 응답 필수 필드 존재 확인. 503·404면 서버 기동 상태와 라우터 등록 먼저 점검.

#### 4. Smoke — 브라우저 통합 확인
```bash
pnpm test:smoke
```
또는 Playwright MCP로 변경된 탭·컨트롤 직접 클릭 → 콘솔 에러 0건 확인.

**결과 기록** (`_workspace/phase_<N>_check.md`):
```
1. Unit:     ✅ N/N 통과   |  ❌ 실패: <이유>
2. 생성물:   ✅ 확인       |  ❌ <항목> 누락·오형
3. Endpoint: ✅ 200 OK     |  ❌ <status> <에러>
4. Smoke:    ✅ 에러 0건   |  ❌ <에러 목록>
```
